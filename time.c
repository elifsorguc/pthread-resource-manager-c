...
void *threadfunc1(void *a)
{
    ...
    if (reman_request(request1) == 0)
    {
        ...
        printf("Thread %d: reman_request took %.6f ms\n", tid, calculate_elapsed_time(start, end));
    ...
    {
        clock_gettime(CLOCK_MONOTONIC, &end); // End timing
        printf("Thread %d: reman_request for request2 took %.6f ms\n", tid, calculate_elapsed_time(start, end));
...
int main(int argc, char **argv)
{
    ...
    while (count < 20)
    {
        ...
        struct timespec start, end;
        clock_gettime(CLOCK_MONOTONIC, &start); // Start timing
        int ret = reman_detect();
        clock_gettime(CLOCK_MONOTONIC, &end); // End timing

        printf("Deadlock detection took %.6f ms\n", calculate_elapsed_time(start, end));
