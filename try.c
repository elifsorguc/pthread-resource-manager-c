...
void *threadfunc1(void *a)
{
    ...
    tid = *((int *)a);
    reman_connect(tid);
    setarray(claim, NUMR, 2, 1, 1); // Claim resources
    reman_claim(claim);

    setarray(request1, NUMR, 2, 0, 0); // Request a subset of resources
    pr(tid, "REQ", NUMR, request1);
    reman_request(request1);
    sleep(3);

    setarray(request2, NUMR, 0, 1, 1); // Request additional resources
...
void *threadfunc2(void *a)
{
    ...
    tid = *((int *)a);
    reman_connect(tid);
    setarray(claim, NUMR, 1, 2, 1); // Claim resources
    reman_claim(claim);

    setarray(request1, NUMR, 0, 2, 0); // Request a subset of resources
    pr(tid, "REQ", NUMR, request1);
    reman_request(request1);
    sleep(2);

    setarray(request2, NUMR, 1, 0, 1); // Request additional resources
    pr(tid, "REQ", NUMR, request2);
    reman_request(request2);
...
void *threadfunc3(void *a)
{
    ...
    tid = *((int *)a);
    reman_connect(tid);
    setarray(claim, NUMR, 1, 1, 2); // Claim resources
    reman_claim(claim);

    setarray(request1, NUMR, 1, 1, 1); // Request resources
    pr(tid, "REQ", NUMR, request1);
    reman_request(request1);
    sleep(4);
